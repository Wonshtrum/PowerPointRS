...
use crate::{
    filters::{DoubleFilter, CELL_SHIFT},
    render::Canvas,
...
fn apply_effect(effect: &mut BasicAnimation, state_dyn: &mut DoubleFilter) -> bool {
    let target = effect.target;
    let (old_visibility, old_targeting) = state_dyn.get(target);
    let visible = match effect.visible {
        Change::True => true,
        Change::False => false,
        Change::Same => old_visibility,
    };
    let targeted = match effect.targeted {
        Change::True => true,
        Change::False => false,
        Change::Same => old_targeting,
    };
    state_dyn.set(target, visible, targeted);
    old_visibility != visible || (visible && old_targeting != targeted)
}
